...
### v1.0.1 — Patch
* Changed date input format from `YYYY-MM-DD` to `DD-MM-YYYY`
* Updated date display format in view to match `DD-MM-YYYY`

### v1.1.0 — Current
* Added `status` column with default value `Active` on every new entry
* Replaced Remove Subscription with **Cancel Subscription**
  * Cancelled subscriptions update status to `Cancelled` instead of being deleted
* AUTO_INCREMENT resets automatically when all subscriptions are removed
* Fixed connection leak — cursor and connection now close properly in all code paths
...
| Version | Description                                      |
| ------- | ------------------------------------------------ |
| v1.0.0  | Initial release with core CRUD features          |
| v1.0.1  | Date format fix (DD-MM-YYYY)                     |
| v1.1.0  | Cancel subscription + status column (current)    |
---
## 👤 Author
Ranjan Verma
